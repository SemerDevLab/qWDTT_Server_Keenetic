...
qWDTT — клиент и сервер защищённого туннеля для роутеров Keenetic с Entware. В релизах проекта доступны готовые IPK-пакеты для поддерживаемых архитектур Keenetic.
...
Готовые IPK-пакеты для всех поддерживаемых архитектур находятся в разделе [Releases](https://git.semer.fun/mikhail.semerkov/qWDTT_Keenetic/releases). Выберите нужную версию и скачайте пакет согласно архитектуре роутера:
...
1. Откройте нужный релиз в разделе [Releases](https://git.semer.fun/mikhail.semerkov/qWDTT_Keenetic/releases) и скачайте пакет, соответствующий архитектуре роутера.
2. Скопируйте скачанный файл на роутер, например в `/opt/tmp`:

```sh
scp qwdtt_0.1.0-20_aarch64-3.10-kn.ipk root@ROUTER:/opt/tmp/
```

3. Подключитесь к роутеру и установите пакет:

```sh
opkg install /opt/tmp/qwdtt_0.1.0-20_aarch64-3.10-kn.ipk
```
...
Скачайте новую версию из раздела [Releases](https://git.semer.fun/mikhail.semerkov/qWDTT_Keenetic/releases) и установите её поверх текущей:
...
Лицензия проекта пока не задана отдельным файлом. Перед публичной публикацией проекта добавьте подходящую лицензию в `LICENSE`.
